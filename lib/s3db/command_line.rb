...
module S3db
  class CommandLine
    ...
    def mysql_params
      ...
      command_chain << "--login-path=#{config.db['login_path']}" if config.db['login_path']
      command_chain << "--password='#{config.db['password']}'" if config.db['password'] && !config.db['login_path']
